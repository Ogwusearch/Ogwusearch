...
import { PV_ARRAY_CONSTANTS } from "../constants.js";
import { PV_ARRAY_ERROR_CODES } from "../errors.js";
import type { PvArrayInput } from "../types/index.js";
import { PV_ARRAY_WARNING_CODES } from "../warnings.js";

export function validatePvArray(
  input: PvArrayInput,
): EngineeringIssue[] {
  ...
  if (!Number.isFinite(input.modulePowerW)) {
    ...
    input.modulePowerW <=
    PV_ARRAY_CONSTANTS.minModulePowerW
  ) {
    addError(
      PV_ARRAY_ERROR_CODES.INVALID_MODULE_POWER,
  ...
  if (!Number.isFinite(input.moduleVmpV)) {
    ...
    input.moduleVmpV <=
    PV_ARRAY_CONSTANTS.minModuleVoltageV
  ) {
    addError(
      PV_ARRAY_ERROR_CODES.INVALID_MODULE_VMP,
  ...
  if (!Number.isFinite(input.moduleImpA)) {
    ...
    input.moduleImpA <=
    PV_ARRAY_CONSTANTS.minModuleCurrentA
  ) {
    addError(
      PV_ARRAY_ERROR_CODES.INVALID_MODULE_IMP,
